...
class UserController extends Controller {
  ...
  async verifyUser(req, res) {
    ...
    if (updateResult.modifiedCount === 0)
      throw createHttpError.InternalServerError("User status not updated.");

    let message = "User status confirmed.";
    if (status === 0) message = "User status changed to rejected.";
    if (status === 1) message = "User status changed to pending confirmation.";
